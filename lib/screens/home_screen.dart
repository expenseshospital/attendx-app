...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _markDutyOut() async {
    ...
    setState(() { _isDutyOn = false; _isLoading = false; });
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('AttendX', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        actions: [
          Padding(
            padding: const EdgeInsets.only(right: 16),
            child: Row(children: [
              Icon(Icons.circle, size: 12, color: _isInsideGeofence ? Colors.green : Colors.red),
              const SizedBox(width: 4),
              Text(_isInsideGeofence ? 'Inside' : 'Outside', style: const TextStyle(color: Colors.white)),
            ]),
          ),
        ],
      ),
      ...
      child: Column(children: [
        Container(
          ...
          decoration: BoxDecoration(
            gradient: const LinearGradient(colors: [Color(0xFF1D6FE8), Color(0xFF0A4FB4)]),
            borderRadius: BorderRadius.circular(20),
          ),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Good ${_getGreeting()}!', style: const TextStyle(color: Colors.white70, fontSize: 16)),
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: _isInsideGeofence ? Colors.green : Colors.red),
          ),
          child: Row(children: [
            Icon(_isInsideGeofence ? Icons.location_on : Icons.location_off,
              color: _isInsideGeofence ? Colors.green : Colors.red, size: 30),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(_isInsideGeofence ? 'You can mark attendance' : 'Move to office',
                style: TextStyle(fontSize: 12, color: Colors.grey[600])),
            ])),
            IconButton(onPressed: _checkLocation, icon: const Icon(Icons.refresh)),
          ]),
        ),
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              backgroundColor: _isDutyOn ? Colors.red : Colors.green,
